...
float TurboKeepTime[40];
boolean SmashButton[40];
float TurboDelay=50;
float TurboKeepTimeMax=5000; // Time to keep the button + pin_SwitchPadAnalog to force A+B Smash button
...
dataForController_t getControllerData(void){
  ...
  if (!digitalRead(pin_SwitchPadAnalog)  && AnyButtonPressed() )
  {
    for (int i=pin_X; i<pin_SWITCH_ZR+1; ++i)
    {
      ...
      if ( !digitalRead (i) && !TurboKeep[i] )  // First time pushed, triggered
      {
        ...
        TurboKeepTime[i]=millis();
        SmashButton[i]=false;
        
#ifdef DEBUG  
  Serial.println("Button TURBO");
  Serial.println(i);
  ...
  Serial.println("-----------------------------");    
      ...
      {
        if ( !digitalRead (i) && TurboKeep[i] && (millis() - TurboKeepTime[i] > TurboKeepTimeMax) )  // If keep more than TurbotKeepTimeMax then this button is A+B
        {
          TurboActivate[i]=false;
          SmashButton[i]=true; 

#ifdef DEBUG  
  Serial.println("Button SMASHBOX ACTIVATED");
  Serial.println(i);
  Serial.println(TurboActivate[i]);
  Serial.println("-----------------------------");  
#endif
...
///////////////////////////////////////////////////////////////////////////////////////
  ...
  if ( LastStatusButtonIDJ && !digitalRead(pin_SpecialReplicateButton) ) // IDJ IN PROCESS (NO FIRST FRAME)
  {
    ...
    if  ( IDJ_delta < IDJ_threshold_1 )                                   { controllerData.crossOn=true; digitalWrite(pin_SpecialReplicateButton,HIGH);    }
    if  ( ( IDJ_delta >= IDJ_threshold_1 ) && ( IDJ_delta < IDJ_threshold_2 ) )  { controllerData.crossOn=false; digitalWrite(pin_SpecialReplicateButton,LOW); }
    if  ( IDJ_delta >= IDJ_threshold_2 )                                  { controllerData.crossOn=true; digitalWrite(pin_SpecialReplicateButton,HIGH);   }    
  }
...
void TurnOffAllLeds(void)
{
  ...
  digitalWrite(Led_pin_DoubleJump,LOW);
}
...
void TurnOnAllLeds(void)
{
  ...
  digitalWrite(Led_pin_DoubleJump,HIGH);
